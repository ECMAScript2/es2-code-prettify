...
PR['registerLangHandler'](
    PR['createSimpleLexer'](
        [
         ...
         // @attributes
         [PR['PR_ATTRIB_NAME'], new RegExpCompat( "^@\\w+" ), null, '@'],
         [PR['PR_PUNCTUATION'], new RegExpCompat( "^[!%&()*+,\\-;<=>?\\[\\\\\\]^{|}]+" ), null,
          '!%&()*+,-;<=>?[\\]^{|}'],
         // Borrowed from lang-erlang.js:
         [PR['PR_LITERAL'],
          new RegExpCompat( "^(?:0o[0-7](?:[0-7]|_[0-7])*|0x[\\da-fA-F](?:[\\da-fA-F]|_[\\da-fA-F])*|\\d(?:\\d|_\\d)*(?:\\.\\d(?:\\d|_\\d)*)?(?:[eE][+\\-]?\\d(?:\\d|_\\d)*)?)" ),
          null, '0123456789']
        ],
        [
         // the iex> prompt for interactive examples
         [PR['PR_ATTRIB_NAME'], new RegExpCompat( "^iex(?:\\(\\d+\\))?> " )],
         ...
         [PR['PR_LITERAL'], new RegExpCompat( "^:(?:\\w+[\\!\\?\\@]?|\"(?:[^\"\\\\]|\\\\.)*\"?)" )],
         // compile-time information
         [PR['PR_ATTRIB_NAME'], new RegExpCompat( "^(?:__(?:CALLER|ENV|MODULE|DIR)__)" )],
         // keywords
         [PR['PR_KEYWORD'],
          new RegExpCompat( "^(?:alias|case|catch|def(?:delegate|exception|impl|macrop?|module|overridable|p?|protocol|struct)|do|else|end|fn|for|if|in|import|quote|raise|require|rescue|super|throw|try|unless|unquote(?:_splicing)?|use|when|with|yield)\\b" )],
         [PR['PR_LITERAL'], new RegExpCompat( "^(?:true|false|nil)\\b" )],
         ...
         [PR['PR_COMMENT'], new RegExpCompat( "^_\\w*" )],
         // plain: variables, functions, ...
         [PR['PR_PLAIN'], new RegExpCompat( "^[$a-z]\\w*[\\!\\?]?" )],
         // sigils with the same starting and ending character.
         // Key part: X(?:[^X\r\n\\]|\\.)+X where X is the sigil character
         [PR['PR_ATTRIB_VALUE'], new RegExpCompat( "^~[A-Z](?:\\/(?:[^\\/\\r\\n\\\\]|\\\\.)+\\/|\\|(?:[^\\|\\r\\n\\\\]|\\\\.)+\\||\"(?:[^\"\\r\\n\\\\]|\\\\.)+\"|'(?:[^'\\r\\n\\\\]|\\\\.)+')[A-Z]*", 'i' )],
