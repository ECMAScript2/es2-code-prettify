...
var fallthroughStylePatterns = [
         ...
         [PR_DECLARATION, RegExpProxy('^(?:\\$(?:' + intrinsic + '))\\b', 'i'), null],
         // Add commands as keywords
         [PR_KEYWORD, RegExpProxy('^(?:[^\\$]' + commands + ')\\b', 'i'), null],
         // A number is a decimal real literal or in scientific notation. 
         [PR_LITERAL,
          RegExpProxy( "^[+-]?(?:(?:\\.\\d+|\\d+(?:\\.\\d*)?)(?:E[+\\-]?\\d+)?)", 'i' )], 
         // An identifier
         [PR_PLAIN, RegExpProxy( "^[a-z][a-zA-Z0-9]*", 'i' )],
         // Exclude $ % and ^
         [PR_PUNCTUATION, RegExpProxy( "^[^\\w\\t\\n\\r\\xA0\\\"\\$;%\\^]|_" )]
  ];
// Can't use m as its already used for MatLab
registerLangHandler(createSimpleLexer(shortcutStylePatterns, fallthroughStylePatterns), ['mumps']);
})();
